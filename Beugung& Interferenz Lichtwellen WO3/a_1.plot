reset

set xrange[-0.65:0.65]
#set yrange[-0.001:1.0172]

set xlabel "Winkel/Grad"
set ylabel "Intensitaetsverhältniss"

#f(x) = (sin(((3.14 * 0.2 * sin((x-0)/9))/(0.055)))/((3.14 * sin((x-0)/9) * 0.2)/(0.055)))**2
g(x) = (sin(((3.14 * 0.02 * sin((x-0)))/(0.0006328)))/((3.14 * sin((x-0)) * 0.02)/(0.0006328)))**2

set sample 1000000

#fit [1:14] g(x) "a_1.dat" using 1:2:3 via a,b,c

set xtics 0.1

plot "a_1.dat" with xyerrorbars title "Messdaten"  smooth csplines , g(x) title "Fit"#, f(x) title "Regressionskurve"

set terminal postscript eps enhanced color font 'Helvetica,10'
set output "a_1.eps"

replot 

! epstopdf a_1.eps
